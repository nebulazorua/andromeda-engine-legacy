...
class PlayState extends MusicBeatState
{
	...
	function setupLuaSystem()
	{
		...
		if (luaModchartExists)
		{
			...
			Lua_helper.add_callback(lua.state, "compensateFPS", function(num:Float)
			{ // prob need new name? idk
			...
			Lua_helper.add_callback(lua.state, "newSprite",
				function(?x:Int = 0, ?y:Int = 0, ?drawBehind:Bool = false, ?autoAdd:Bool = false, ?spriteName:String)
				{
			...
			defaultLuaClasses = [
				luaModchart, window, bfLua, gfLua, dadLua, bfIcon, dadIcon, luaGameCam, luaHUDCam, luaNotesCam, luaSustainCam, luaReceptorCam, luaRenderedNotes
			];
	...
	override public function create()
	{
		...
		if (inCharter)
		{
			if (currentOptions.chartingBotplay)
				currentOptions.botPlay = true;
	...
	private function generateSong():Void
	{
		...
		if (dynamicColouring == null)
			dynamicColouring = false;
		...
		var func = Math.round;
		if (!currentOptions.fixHoldSegCount)
			func = Math.floor;
		for (section in noteData)
		{
				...
				for (event in section.events)
				{
					if (event.events != null)
					{
						...
						for (ev in event.events)
						{
							...
							for (e in pushingEvents)
								eventSchedule.push(e);
			...
			for (songNotes in section.sectionNotes)
			{
				...
				var swagNote:Note = new Note(daStrumTime, daNoteData, currentOptions.noteSkin, noteModifier, EngineData.noteTypes[songNotes[3]], oldNote,
					false, songNotes[4] == true, getPosFromTime(daStrumTime));
				...
				if (Math.round(susLength) > 0)
				{
					for (susNote in 0...Math.round(susLength))
					{
						...
						var sustainNote:Note = new Note(sussy, daNoteData, currentOptions.noteSkin, noteModifier, EngineData.noteTypes[songNotes[3]], oldNote,
							true, swagNote.isRoll, getPosFromTime(sussy));
	...
	function updateCamFollow()
	{
		...
		else
		{
			...
			switch (focus)
			{
				...
				case 'center':
					focusedChar = null;
					var centerX = (stage.centerX == -1) ? (((dadMid.x + opponent.camOffset.x) + (bfMid.x - stage.camOffset.x +
						boyfriend.camOffset.x)) / 2) : stage.centerX;
					var centerY = (stage.centerY == -1) ? (((dadMid.y + opponent.camOffset.y) + (bfMid.y - stage.camOffset.y +
						boyfriend.camOffset.y)) / 2) : stage.centerY;
	...
	override public function update(elapsed:Float)
	{
		...
		if (accuracy < 1 && shownAccuracy == 100)
			shownAccuracy = 99.99;
			...
			if (health <= 0)
			{
				if (!currentOptions.noFail && !inCharter)
				{
					...
					if (boyfriend.animation.getByName("firstDeath") != null
						&& boyfriend.animation.getByName("deathLoop") != null
						&& boyfriend.animation.getByName("deathConfirm") != null)
						char = boyfriend.curCharacter;
		...
		if (generatedMusic)
		{
			if (startedCountdown)
			{
				...
				renderedNotes.forEachAlive(function(daNote:Note)
				{
					...
					if (daNote.holdingTime < daNote.sustainLength && daNote.mustPress)
					{ // NEW HOLD LOGIC
					...
					var shitGotHit = (daNote.parent != null && daNote.parent.wasGoodHit && daNote.canBeHit)
		...
		if (combo != 0)
		{
			if (currentOptions.showComboCounter)
			{
				...
				for (i in seperatedScore)
				{
					...
					if (judgeMan.judgementCounter.get("miss") == 0
						&& judgeMan.judgementCounter.get("bad") == 0
						&& judgeMan.judgementCounter.get("shit") == 0)
					{
	...
	private function keyRelease(event:KeyboardEvent)
	{
		if (ScoreUtils.botPlay)
			return;
		var direction = bindData.indexOf(event.keyCode);
		if (paused && (direction == -1 || !pressedKeys[direction]))
			return;
		if (direction != -1 && pressedKeys[direction])
		{
	...
	private function preNoteLogic(elapsed:Float)
	{
		...
		if (ScoreUtils.botPlay)
		{
			...
			{
				if (botplayHoldTimes[dir] > 0)
					botplayHoldTimes[dir] -= elapsed * 1000;
				if (botplayHoldTimes[dir] < 0)
					botplayHoldTimes[dir] = 0;
	...
	function getHittableNotes(direction:Int = -1, excludeHolds:Bool = false)
	{
		...
		{
			if ((note.canBeHit && note.alive && !note.wasGoodHit || note.sustainLength > 0 && note.wasGoodHit)
				&& !note.tooLate
				&& (direction == -1 || note.noteData == direction)
				&& (!excludeHolds || !note.isSustainNote))
				notes.push(note);
		}
		return notes;
	}
