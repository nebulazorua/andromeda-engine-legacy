...
class CachingState extends FlxUIState
{
	...
	override function create()
	{
		...
		if (EngineData.options.cachePreload)
		{
			if (FileSystem.isDirectory("assets/images"))
			{
				for (file in FileSystem.readDirectory("assets/images"))
				{
					...
					{ // TODO: recursively go through all directories
		...
		bar = new FlxBar(barBG.x + 4, barBG.y + 4, LEFT_TO_RIGHT, Std.int(barBG.width - 8), Std.int(barBG.height - 8), this, 'loaded', 0, toLoad);
		...
		loadSound = Thread.create(() ->
		{
			while (true)
			{
				...
				if (msg != null && msg != recentlyLoadedSnd)
				{
					...
					if (Assets.exists(msg, AssetType.SOUND) || Assets.exists(msg, AssetType.MUSIC))
