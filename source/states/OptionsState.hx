	...
	private function createDefault()
	{
		defCat = new OptionCategory("Default", [
			new OptionCategory("Gameplay", [
				...
				#if !NO_FREEPLAY_MODS
				new ToggleOption("fixHoldSegCount", "Hold Segment Count Fix",
					"Fixes a bug where holds are smaller than they should be.\nMay cause holds to be longer than they should in old charts."),
				new OptionCategory("Freeplay Modifiers", [
					new StepOption("cMod", "Speed Constant", 0.1, 0, 10, "", "", "A constant speed to override the scrollspeed. 0 for chart-dependant speed",
						true),
				...
				new OptionCategory("Advanced", [
					#if !FORCED_JUDGE
					new JudgementsOption("judgementWindow", "Judgements",
						"The judgement windows to use"), new ToggleOption("useEpic", "Use Epics", "Allows the 'Epic' judgement to be used"),
					#end
			...
			new OptionCategory("Performance", [
				new StepOption("fps", "FPS Cap", 30, 30, 360, "", "", "The FPS the game tries to run at", true,
					function(value:Float, step:Float)
					{
						Main.setFPSCap(Std.int(value));
					}),
				new ToggleOption("recycleComboJudges", "Recycling",
				...
				new ToggleOption("allowOrderSorting", "Sort notes by order",
