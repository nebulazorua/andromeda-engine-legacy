...
class Note extends NoteGraphic
{
	...
	public function new(strumTime:Float, noteData:Int, skin:String = 'default', modifier:String = 'base', type:String = 'default', ?prevNote:Note,
			?sustainNote:Bool = false, ?rollNote:Bool = false, ?initialPos:Float = 0, ?beingCharted = false)
	{
		this.noteType = type;
		isRoll = rollNote;
		...
		if (modBehaviours == null)
			modBehaviours = new Map<String, NoteBehaviour>();
