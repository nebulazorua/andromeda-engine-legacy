...
class NoteGraphic extends FNFSprite
{
	...
	public var noteOffsets:Array<Array<Null<Float>>> = [[0, 0], [0, 0], [0, 0], [0, 0]];
	...
	public var quantToGrid:Map<Int, Int> = [4 => 0, 8 => 1, 12 => 2, 16 => 3, 24 => 4, 32 => 5, 48 => 6, 64 => 7, 192 => 8,];
	...
	public function setTextures()
	{
		switch (behaviour.actsLike)
		{
			case 'pixel':
				...
				loadGraphic(Paths.noteSkinImage(behaviour.arguments.note.sheet, 'skins', skin, modifier, graphicType), true,
					behaviour.arguments.note.gridSizeX, behaviour.arguments.note.gridSizeY);
			...
			default:
				...
				if (args.quants != null && field != null)
				{
					...
					for (i in 0...dirs.length)
					{
						...
						if (prefix == null)
							prefix = field.prefix;
						if (longPrefix == null)
							longPrefix = field.longPrefix;
						if (longEndPrefix == null)
							longEndPrefix = field.longEndPrefix;
						if (rollPrefix == null)
							rollPrefix = field.rollPrefix;
						if (rollEndPrefix == null)
							rollEndPrefix = field.rollEndPrefix;

						if (prefix == null)
							prefix = Reflect.field(args, dirs[i]).prefix;
						if (longPrefix == null)
							longPrefix = Reflect.field(args, dirs[i]).longPrefix;
						if (longEndPrefix == null)
							longEndPrefix = Reflect.field(args, dirs[i]).longEndPrefix;
						if (rollPrefix == null)
							rollPrefix = Reflect.field(args, dirs[i]).rollPrefix;
						if (rollEndPrefix == null)
							rollEndPrefix = Reflect.field(args, dirs[i]).rollEndPrefix;
						...
						noteOffsets[i] = [
							dirData.xOffset == null ? 0 : dirData.xOffset,
							dirData.yOffset == null ? 0 : dirData.yOffset
						];
					}
				}
				else
				{
					...
					noteOffsets[0] = [
						args.left.xOffset == null ? 0 : args.left.xOffset,
						args.left.yOffset == null ? 0 : args.left.yOffset
					];
					noteOffsets[1] = [
						args.down.xOffset == null ? 0 : args.down.xOffset,
						args.down.yOffset == null ? 0 : args.down.yOffset
					];
					noteOffsets[2] = [
						args.up.xOffset == null ? 0 : args.up.xOffset,
						args.up.yOffset == null ? 0 : args.up.yOffset
					];
					noteOffsets[3] = [
						args.right.xOffset == null ? 0 : args.right.xOffset,
						args.right.yOffset == null ? 0 : args.right.yOffset
					];
				}
	...
	public function setDir(dir:Int = 0, ?type:Int = 0, ?end:Bool = false)
	{
		...
		if ((sussy || roll) && behaviour.actsLike == 'pixel')
		{
			if (animation.curAnim != null)
			{
				if (end && !animation.curAnim.name.endsWith("end"))
				{
					...
					if (roll)
						args = behaviour.arguments.rollEnd;
		...
		{
			loadGraphic(Paths.noteSkinImage(behaviour.arguments.note.sheet, 'skins', skin, modifier, graphicType), true, behaviour.arguments.note.gridSizeX,
				behaviour.arguments.note.gridSizeY);
