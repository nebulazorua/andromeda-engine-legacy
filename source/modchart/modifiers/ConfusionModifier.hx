...
class ConfusionModifier extends Modifier
{
	override function updateNote(note:Note, player:Int, pos:Vector3, scale:FlxPoint)
	{
		...
		{
			note.modAngle = (getPercent(player) + getSubmodPercent('confusion${note.noteData}',
				player) + getSubmodPercent('note${note.noteData}Angle', player)) * 100;
		}
	}

	override function updateReceptor(receptor:Receptor, player:Int, pos:Vector3, scale:FlxPoint)
	{
		receptor.desiredAngle = (getPercent(player)
			+ getSubmodPercent('confusion${receptor.direction}', player)
			+ getSubmodPercent('receptor${receptor.direction}Angle', player)) * 100;
	}
