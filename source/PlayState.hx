...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		unnamedLuaObjects=0;
		currentPState=this;
		ScoreUtils.ratingWindows = OptionUtils.ratingWindowTypes[Options.ratingWindow];
		...
		if(Options.ratingWindow!=0){
			presetTxt.text = OptionUtils.ratingWindowNames[Options.ratingWindow] + " Judgement";
			presetTxt.x = 0;
		...
		startingSong = true;
		if(modchartExists && Options.loadModcharts){
			lua = new LuaVM();
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			{
				var daStrumTime:Float = songNotes[0] + Options.noteOffset;
	...
	override public function update(elapsed:Float)
	{
		...
		if(misses>0 && Options.failForMissing){
			health=0;
		}
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if(Options.downScroll){
					...
					if(daNote.animation.curAnim.name.endsWith("end") && daNote.prevNote!=null && daNote.prevNote.scale!=null){
				...
				if (!daNote.mustPress && daNote.canBeHit && !daNote.wasGoodHit)
				{
					...
					if(modchartExists && lua!=null){
						...
						}else if(Options.pauseHoldAnims && !canHold){
							dad.playAnim(anim,true);
					...
					trace(daNote.y,strumLine.y+Note.swagWidth);
					if(!daNote.isSustainNote || daNote.y > strumLine.y+Note.swagWidth){
						daNote.kill();
				...
				if ((!Options.downScroll && daNote.y < -daNote.height || Options.downScroll && daNote.y>FlxG.height) && daNote.mustPress)
				{
		...
		if(Conductor.songPosition-Options.noteOffset>=FlxG.sound.music.length && FlxG.sound.music.volume>0){
			endSong();
		}
		#if debug
	...
	private function keyShit():Void
	{
		...
		var hasHeld = [false,false,false,false];

		if((left || right || up || down) && generatedMusic ){
			notes.forEachAlive( function(daNote:Note){
				if(daNote.isSustainNote && daNote.canBeHit && daNote.mustPress && holdArray[daNote.noteData] && hasHeld[daNote.noteData]==false){
					trace("HITTING " + daNote.noteData);
					hasHeld[daNote.noteData]=true;
					goodNoteHit(daNote);
				}
			});
		};
		...
		if ((upP || rightP || downP || leftP) && generatedMusic)
			{
				...
				var ignoreList = [];
				var what = [];
				notes.sort((dumpy, a, b) -> Std.int(a.strumTime - b.strumTime));
				notes.forEachAlive( function(daNote:Note){
				...
				if(perfectMode){
					...
					for (idx in 0...controlArray.length){
						...
						if(pressed && ignoreList.contains(idx)==false && Options.ghosttapping==false )
							badNoteCheck();
					}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			if (!note.isSustainNote)
			{
				...
				trace(note.strumTime,Conductor.songPosition);
				popUpScore(noteDiff);
			...
			vocals.volume = 1;
			if (!note.isSustainNote || note.y>strumLine.y+Note.swagWidth)
			{
				note.kill();
	...
	function updateTrainPos():Void
	{
		...
		if (startedMoving)
		{
			camGame.shake(.001,.1,null,true,X);
			phillyTrain.x -= 400;
	...
	override function stepHit()
	{
		...
		if (FlxG.sound.music.time-Options.noteOffset > Conductor.songPosition-Options.noteOffset + 20 || FlxG.sound.music.time-Options.noteOffset < Conductor.songPosition-Options.noteOffset - 20)
		{
			resyncVocals();
		}
